  ...
  private async execute(job: RunJob): Promise<void> {
    ...
    try {
      ...
      if (job.mode !== 'mock' && !cred.apiKey) {
        ...
        emit({ type: 'warn', message: 'provider_key_required: 执行期未解析到可用 LLM Key' });
        audit({
          tenantId: job.tenantId,
          ...
          outcome: 'denied',
          target: route?.agentId ?? job.agentId ?? 'default',
          detail: { reason: 'provider_key_required', mode: job.mode, domain: job.domain ?? null },
        });
      ...
      const effectiveApiKeys =
        cred.apiKeys && cred.apiKeys.length ? cred.apiKeys : effectiveApiKey ? [effectiveApiKey] : [];
