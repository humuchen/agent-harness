 ...
 * 若 memoryScoring 特性开启，会传入 scorer 用于重要性评估。
 */
...
export async function assembleAgent(
  ...
  ctxWindow?: number,
  /**
   * P2.4 多 Key：本次 run 可用的全部明文 Key（主 Key + 附加 Key）。
   * 长度 > 1 时以多 Key 负载/故障转移策略构造 LLM（按 Key 隔离熔断 + round-robin）；
  ...
  // P1-③：注册 delegate_task 子 agent 工具（独立记忆窗口 + 独立工具循环）。
  // 仅在模式支持时注册（mock/real/real-mcp 均可），工具调用失败不影响主流程。
  registerSubAgentTool(tools, {
    mode,
  ...
  const teamManager = initTeamManager(agentRegistry);
  void teamManager; // teamManager 供 workflow engine 使用
  ...
  if (mode === 'mock') {
    ...
    // BYOK：凭据由 server 按用户解析后 per-run 注入（modelApiKey），绝不读 process.env.OPEN_API_KEY，
    // 避免并发多 run 串号。无 Key → 提示去「设置 → 模型服务商」配置，而非读环境。
    if (!modelApiKey) {
      throw new Error(
        '真实模式需要有效的 LLM API Key：请到「设置 → 模型服务商」填入你的 OpenRouter Key，' +
          '或在「模型」面板添加带 Key 的自定义模型。'
      );
    }
    ...
    // P2.4 多 Key：若解析出多于 1 把 Key，则以多 Key 负载/故障转移策略构造 primary
    // （按 Key 隔离熔断 + round-robin）；否则沿用单 Key 构造（行为不变）。
    const multiKeys =
      Array.isArray(apiKeys) && apiKeys.length > 1 ? apiKeys : [];
    const primaryBase = {
    ...
    const primary =
      multiKeys.length > 0
        ? createMultiKeyLLM(multiKeys, primaryBase)
        : createOpenRouterLLM({
            ...primaryBase,
  ...
  // P1-③：delegate_task 子 agent工具已注册，供 LLM 调用。
  notes.push('已注册 delegate_task 工具（SubAgent 子任务分发）。');

  // P1-④：Agent Teams 管理器已初始化，供 workflow engine 使用。
  notes.push('已初始化 Agent Teams 管理器（/api/teams CRUD）。');
  ...
  // P2.c：记忆打分机制 — 按重要性评估驱动滑动窗口淘汰优先级与长期记忆相关性排序。
  // 默认关闭；MEMORY_SCORING=true 开启（经特性开关框架统一判定）。
  // 零依赖：沿用 HeuristicMemoryScorer，基于词重叠 + 角色权重 + 时效性打分。
  const enableMemoryScoring = isEnabled('memoryScoring');
  const memoryNotesTopK = Number(process.env.MEMORY_NOTES_TOPK ?? 10) || 10;
  let memoryScorer: MemoryScorer | undefined;
  if (enableMemoryScoring) {
    memoryScorer = createHeuristicScorer();
    notes.push(
      `已启用记忆打分机制（heuristic）：滑动窗口溢出时按重要性评估淘汰，` +
      `长期记忆按相关性排序并裁剪到 Top ${memoryNotesTopK}。`
    );
  }
