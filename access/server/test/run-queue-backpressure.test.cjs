...
test('排队背压：队列达上限后 submit 拒绝（同会话串行化维持堆积）', () => {
  ...
  try {
    ...
    mk(2); // queued（会话被占）
    mk(3); // queued，此时 queue.length === 2 === 上限
    assert.throws(() => mk(4), (e) => {
...
test('幂等去重：同 owner 同键活跃任务复用；不同 owner 不受影响', () => {
  const q = new RunQueue();
  try {
    const a = q.submit({ mode: 'mock', prompt: 'p', owner: 'u1', idempotencyKey: 'k1' });
    const b = q.submit({ mode: 'mock', prompt: 'p-again', owner: 'u1', idempotencyKey: 'k1' });
    assert.strictEqual(b.id, a.id, '同键活跃任务应复用既有 job');
    const c = q.submit({ mode: 'mock', prompt: 'p', owner: 'u2', idempotencyKey: 'k1' });
    assert.notStrictEqual(c.id, a.id, '不同 owner 同键不互相去重');
    const d = q.submit({ mode: 'mock', prompt: 'p', owner: 'u1' });
...
test('幂等索引清理：任务进入终态后，同键新提交不再被拦截', async () => {
  const q = new RunQueue();
  try {
    const a = q.submit({ mode: 'mock', prompt: 'p', owner: 'u1', idempotencyKey: 'fin' });
    ...
    const b = q.submit({ mode: 'mock', prompt: 'p2', owner: 'u1', idempotencyKey: 'fin' });
