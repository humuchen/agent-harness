...
/** 读取某 run 的对话记录。 */
export function readTranscript(runKey: string): { role: string; text: string; t: number }[] {
  return dbCall(() => {
    const rows = getDb()
      .prepare('SELECT role, text, created_at FROM ma_transcript WHERE run_key = ? ORDER BY id ASC LIMIT 200')
      .all(runKey);
 ...
 * 返回归集条数。
 */
export function attachRunTranscript(leadId: string, runKey: string): number {
