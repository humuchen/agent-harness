...
import { fullStats, listLeads } from '../store';
...
/**
 * 客资看板视图（前端 Tab）。render() 在 server 侧被 /api/plugins 调用，实时反映共享存储中的
 * 客资漏斗与队列（多副本下任意副本写入均可见）。webapp 仅把返回的 HTML 注入「客资看板」Tab。
 * 约束：render() 无参数、注入经 unsafeHTML（不执行 <script>），交互走原生 HTML
 * （<form method=POST target=_blank> 认领），图表用内联 SVG 渲染。
 */
export const leadDashboardView: PluginUIView = {
  ...
  render(): string {
    ...
    const recs = listLeads().slice(0, 40);
    const funnelStages = [
      'new',
      'contacted',
      'qualified',
      ...
      'arrived',
      'deal',
    ].map((s) => ({ label: s, value: stats.funnel[s as keyof typeof stats.funnel] ?? 0 }));
    ...
    const rows = recs
      .map(
        (r) => `<tr>
          ...
          <td class="ma-snip">${esc(snippet(r.transcript))}</td>
    ...
    return `<div class="ma-dash">
      ...
      <div class="ma-grid">
        ...
        <section class="ma-panel">
          <h3>渠道分布</h3>
          ${channelItems.length ? barChart(channelItems, '#5B8FF9') : '<p class="ma-empty">暂无渠道数据</p>'}
        </section>
        <section class="ma-panel">
          <h3>意向等级分布</h3>
