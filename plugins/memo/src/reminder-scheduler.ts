 ...
 *   - 到期检测：每次 tick 取 pendingReminders(now)（全用户），逐条触发回调（弹通知）。
 *     事件携带 owner（归属用户），宿主 reminder-bus 按订阅连接的 owner 过滤转发；
 *     前端 ack 时才按 owner+id 落库 notified。
 *   - 重启补发：进程重启后（如 Render 杀进程），尚未 notified 且 remindAt 已过的项会被
 *     pendingReminders 自然捞回（notified 持久化在数据库，故重启不丢、不会重复提醒）。
...
export class ReminderScheduler {
  ...
  private async tick(): Promise<void> {
    ...
    this.running = true;
    try {
      const due = await pendingReminders(Date.now());
      for (const n of due) {
        const note = n as MemoNote;
        // 本进程内已 fire 过的不再重复 fire（前端 ack 才落库 notified，避免刷屏）。
