...
/** 看板视图 HTML 经 unsafeHTML 注入到宿主 Lit 组件的 shadow DOM，
 *  内联事件处理器里的 document.getElementById / querySelector 只能查 light DOM，
...
function selectAllBox(): string {
  const js = `Array.from(__r.querySelectorAll('.memo-mgmt-chk')).forEach(function(c){c.checked=event.target.checked})`;
  return `<input type="checkbox" class="memo-mgmt-all" title="全选" onchange="${ROOT_REF}${esc(
    js
  )}">`;
}
...
function batchDelBtn(): string {
  const js = `(function(){var ids=[];var cbs=__r.querySelectorAll('.memo-mgmt-chk:checked');for(var i=0;i<cbs.length;i++){ids.push(cbs[i].value)}if(!ids.length){return}if(!confirm('确认删除选中的 '+ids.length+' 条备忘？此操作不可恢复')){return}fetch('/api/plugins/memo/notes/batch',{method:'DELETE',credentials:'include',headers:Object.assign((function(){var h={};var u=localStorage.getItem('ah_user');if(u){h['x-ah-username']=u}return h})(),{'content-type':'application/json'}),body:JSON.stringify({ids:ids})}).then(function(){${refreshCurrentJs()}})})()`;
  return `<button class="memo-batch-del" onclick="${ROOT_REF}${esc(
    js
  )}">删除选中</button>`;
}
...
const BOARD_FETCH_TAIL = `.then(function(r){return r.json()}).then(function(d){var b=__r.getElementById('memo-mgmt-body');if(b)b.innerHTML=d.html})`;
...
function boardUrl(offsetExpr: string): string {
  return `('/api/plugins/memo/board?offset='+(${offsetExpr})+'&q='+encodeURIComponent((__r.getElementById('memo-search')||{value:''}).value)+'&sort='+encodeURIComponent((__r.getElementById('memo-sort')||{value:'newest'}).value)+(function(){var t=(__r.getElementById('memo-tag')||{value:''}).value;return t?('&tag='+encodeURIComponent(t)):''})())`;
}
...
function refreshCurrentJs(): string {
  return `var __o=__r.getElementById('memo-offset');var __off=__o?parseInt(__o.value||'0',10):0;fetch(${boardUrl(
    '__off'
  )},{credentials:'include',headers:${AUTH_HEADERS_JS}})${BOARD_FETCH_TAIL}`;
}
  ...
  const pager = `
    ...
    offset + count
      ...
      <span class="memo-pager-btns">
        ${
          offset > 0
            ? `<button class="memo-page-btn" onclick="${ROOT_REF}${esc(
                goJs(offset - limit)
              )}">上一页</button>`
  ...
  async render(user?: PluginRouteUser): Promise<string> {
    ...
    <div class="memo-dash">
      ...
      <style>
        ...
        .memo-search { flex:1 1 220px; min-width:160px; height:38px; box-sizing:border-box; background: var(--ah-surface-3); border:1px solid transparent; border-radius:10px; padding:0 14px; color: var(--ah-text); font:inherit; font-size:14px; outline:none; transition:border-color .15s, box-shadow .15s; }
        ...
        .memo-select-trigger {
          ...
          padding:0 12px; color: var(--ah-text); font:inherit; font-size:14px;
          outline:none; cursor:pointer;
          transition:background .15s, border-color .15s, box-shadow .15s;
        }
        .memo-select-trigger:hover { background: var(--ah-surface-3); border-color: var(--ah-border-strong, var(--ah-border)); }
        .memo-select-trigger:focus-visible,
        .memo-select-trigger:focus { border-color: var(--ah-accent); box-shadow:0 0 0 2px var(--ah-accent-alpha, rgba(41,151,255,.3)); }
        .memo-select-trigger-icon { font-size:14px; line-height:1; flex:none; }
        .memo-select-trigger-label { overflow:hidden; text-overflow:ellipsis; white-space:nowrap; }
        .memo-select-trigger-chevron { font-size:10px; color: var(--ah-text-muted); margin-left:auto; opacity:0.7; transition:transform .18s ease; }
        ...
        .memo-select-menu::-webkit-scrollbar { width:8px; }
        .memo-select-menu::-webkit-scrollbar-thumb { background: var(--ah-border); border-radius:8px; }
        .memo-select-option {
          display:flex; align-items:center; gap:8px;
          padding:8px 10px; cursor:pointer; font:inherit; font-size:13px; color: var(--ah-text);
          border-radius:7px; margin:0;
          transition:background .12s;
        }
        .memo-select-option:hover { background: var(--ah-surface-2); }
        .memo-select-option-icon { font-size:14px; line-height:1; flex:none; }
        .memo-select-option-selected { background: var(--ah-accent-alpha, rgba(41,151,255,.15)); color: var(--ah-accent); font-weight:600; }
        .memo-select-option-selected:hover { background: var(--ah-accent-alpha, rgba(41,151,255,.22)); }
        .memo-select-option-selected .memo-select-option-icon { color: var(--ah-accent); }
        .memo-select-option-selected::after { content:'✓'; margin-left:auto; color: var(--ah-accent); font-weight:700; }
