...
test('manifest 形状：id/capabilities/assembly.skills 收窄', () => {
  ...
  assert.ok(
    Array.isArray(m.assembly.skills) &&
      m.assembly.skills.includes('repo-verify'),
...
test('工具注册与读写闭环：save → list → delete（runWithUser 绑定 alice）', async () => {
  ...
  await core.runWithUser({ sub: 'alice' }, async () => {
    // save
    const saved = await reg.call('note_save', {
      text: '今天对接医美看板',
      tag: 'work'
    });
...
test('owner 绑定：alice 保存的备忘 bob 不可见、不可删', async () => {
  ...
  const bobDel = await core.runWithUser(bob, () =>
    reg.call('note_delete', { id: saved.id })
  );
  assert.equal(bobDel.deleted, false, '跨用户删除应被 (owner,id) 收口拒绝');

  // alice 视角：数据仍在
  const aliceList = await core.runWithUser(alice, () =>
    reg.call('note_list', {})
  );
...
test('assembly.skills 收窄：memo agent 只启用 repo-verify', () => {
  ...
  const skills = core
    .defaultSkills()
    .filter((s) => plugin.memoPlugin.manifest.assembly.skills.includes(s.id));
...
test('边界：超大文本（~1MB）往返一致', async () => {
  ...
  assert.equal(saved.ok, true);
  const listed = await core.runWithUser(alice, () =>
    reg.call('note_list', { tag: 'big' })
  );
  assert.equal(listed.total, 1);
...
test('提醒：note_save 拒绝过去/非法 remindAt（忽略提醒，不污染数据）', async () => {
  ...
  const store = require('../dist/store.js');
  assert.equal(
    (await store.pendingReminders(Date.now())).length,
    0,
    '无待提醒项'
  );
});
...
test('提醒：路由 /reminders 返回 pending+upcoming；/reminders/ack 标记', async () => {
  ...
  {
    const c = collect();
    await ext.mountRoutes['/reminders'](
      mkReq('/reminders', 'GET'),
      c.res,
      alice
    );
  ...
  {
    const dueId = (await store.pendingReminders(Date.now())).find(
      (n) => n.owner === alice.sub
    ).id;
    const c = collect();
    await ext.mountRoutes['/reminders/ack'](
      mkReq(`/reminders/ack?id=${dueId}`, 'POST'),
      c.res,
      alice
    );
    assert.equal(c.obj.ok, true);
    assert.equal(c.obj.notified, true, 'ack 应落库');
    const still = (await store.pendingReminders(Date.now())).filter(
      (n) => n.owner === alice.sub
    );
...
test('提醒：调度器到点 fire 一次（进程内去重）+ 重启自然补发（靠 pendingReminders）', async () => {
  ...
  const ReminderScheduler =
    require('../dist/reminder-scheduler.js').ReminderScheduler;
  ...
  assert.equal(
    (await store.pendingReminders(Date.now())).length,
    1,
    '未 ack 时，重启后 pending 仍可被捞回（天然补发）'
  );
...
test('提醒历史：ack 时落库 notifiedAt，reminderHistory 按确认时间倒序返回', async () => {
  ...
  const a = await store.saveNote(
    alice.sub,
    '第一条',
    'work',
    Date.now() - 300_000
  );
  const b = await store.saveNote(
    alice.sub,
    '第二条',
    'work',
    Date.now() - 200_000
  );
  const c = await store.saveNote(
    alice.sub,
    '第三条',
    'life',
    Date.now() - 100_000
...
test('提醒历史：未 ack 的项不进历史；pending/upcoming 也不混入历史', async () => {
  freshDataDir();
  const store = require('../dist/store.js');
  const pending = await store.saveNote(
    alice.sub,
    '到期未 ack',
    'work',
    Date.now() - 1000
  );
  const upcoming = await store.saveNote(
    alice.sub,
    '将来才提醒',
    'work',
    Date.now() + 60_000
  );
  const done = await store.saveNote(
    alice.sub,
    '已确认',
    'work',
    Date.now() - 5000
...
test('提醒历史：无 remindAt 的普通备忘不会误入历史', async () => {
  ...
  assert.equal(
    (await store.reminderHistory(alice.sub, 20)).length,
    0,
    '无 remindAt 的备忘不应出现在历史'
  );
});
...
test('提醒：路由 /reminders 额外返回 history 字段', async () => {
  ...
  assert.equal(
    typeof data.history[0].notifiedAt,
    'number',
    'history 项应带 notifiedAt'
  );
});
...
test('数据管理：searchNotes 关键词/标签过滤 + 分页 + total（均按 owner 收口）', async () => {
  ...
  assert.ok(
    r1.items.every((n) => n.owner === alice.sub),
    '结果不应混入 bob'
  );
...
test('数据管理：noteStats 统计（总数/带标签/含提醒/历史）', async () => {
  ...
  assert.equal(
    s.withReminder,
    2,
    '含提醒 = 2（「有提醒」未来 + 「已提醒」过去 remind_at 均计入）'
  );
...
test('数据管理：deleteNotes 批量删除按 owner 收口（越权 id 忽略），返回实际删除数', async () => {
  ...
  assert.equal(
    (await store.listNotes(alice.sub)).length,
    0,
    'alice 的两条已删'
  );
  assert.equal(
    (await store.listNotes(bob.sub)).length,
    1,
    'bob 的数据不受影响'
  );
...
test('数据管理：路由 /stats 返回统计；/notes 支持 q/offset/total', async () => {
  ...
  {
    const c = collect();
    await ext.mountRoutes['/notes'](
      { url: `/notes?q=${encodeURIComponent('项目')}`, method: 'GET' },
      c.res,
      alice
    );
    assert.equal(c.obj.ok, true);
  ...
  }
  {
    const c = collect();
    await ext.mountRoutes['/notes'](
      { url: '/notes?tag=work&offset=0&limit=1', method: 'GET' },
      c.res,
      alice
    );
...
test('数据管理：路由 /notes/batch 批量删除 + /notes/all 需 confirm', async () => {
  ...
  {
    const c = collect();
    await ext.mountRoutes['/notes/batch'](
      mkReq('/notes/batch', 'DELETE', { ids: [a1.id, a2.id] }),
      c.res,
      alice
    );
    assert.equal(c.obj.ok, true);
  ...
  {
    const c = collect();
    await ext.mountRoutes['/notes/all'](
      mkReq('/notes/all', 'DELETE', {}),
      c.res,
      alice
    );
  ...
  await store.saveNote(alice.sub, 'a3');
  {
    const c = collect();
    await ext.mountRoutes['/notes/all'](
      mkReq('/notes/all', 'DELETE', { confirm: true }),
      c.res,
      alice
    );
    assert.equal(c.obj.ok, true);
...
test('全选 checkbox：使用 onchange 而非 onclick，避免 this 绑定丢失；JS 不含 < 运算符', async () => {
  ...
  assert.ok(
    headerMatch[0].includes('onchange='),
    '全选框应绑定 onchange 而非 onclick'
  );
...
test('下拉菜单：点击选项应正确设置 hidden select value 并触发 change 事件', async () => {
  ...
  assert.ok(
    onclickJs.includes('s.dispatchEvent(new Event'),
    '选项点击应在 select 上派发 change 事件'
  );
...
test('数据管理：searchNotes 支持 sort（newest/oldest/remind）', async () => {
  ...
  const withR = await store.saveNote(
    alice.sub,
    '有提醒',
    'x',
    Date.now() + 60_000
  ...
  assert.ok(
    remind.items[remind.items.length - 1].remindAt == null,
    '无提醒的排最后'
  );
});
...
test('数据管理：路由 /board 返回表体片段 JSON（含 html/total/offset/count），翻页+过滤生效', async () => {
  ...
  {
    const c = collect();
    await ext.mountRoutes['/board'](
      mk('/board?offset=0&limit=2'),
      c.res,
      alice
    );
    assert.equal(c.obj.ok, true);
    ...
    assert.ok(
      c.obj.html.includes('条目 4'),
      'newest 排序下首条应为最新（条目 4）'
    );
  }
  // 第二页 offset=2（条目 2、条目 1）
  {
    ...
    assert.ok(
      c.obj.html.includes('条目 2') || c.obj.html.includes('条目 1'),
      '第二页应包含条目 2/1'
    );
  }
  // 关键词过滤
  {
    const c = collect();
    await ext.mountRoutes['/board'](
      mk(`/board?q=${encodeURIComponent('条目 4')}`),
      c.res,
      alice
    );
...
test('数据管理：路由 /board 支持按标签过滤（tag 参数）', async () => {
...

  {
    const c = collect();
    await ext.mountRoutes['/board'](
      mk(`/board?tag=${encodeURIComponent('work')}`),
      c.res,
      alice
    );
...
test('时区：无时区 ISO 按 Asia/Shanghai 墙上时间解析（UTC 服务器不再偏 8 小时）', async () => {
  ...
  assert.equal(
    aware,
    naive,
    '+08:00 偏移串应与 naive 按 Asia/Shanghai 解析等价'
  );
