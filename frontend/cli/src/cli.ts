...
const optsConfig = {
  base: {
    ...
    default: process.env.AH_BASE ?? 'http://localhost:4173'
  },
  ...
  switch (cmd) {
    ...
    case 'health': {
      try {
        ...
        err(c('red', `unhealthy: ${(e as Error).message}`));
        return 1;
      }
    }
    case 'metrics': {
    ...
    case 'run': {
      ...
      return runStream(
        ...
        (t) =>
          client.streamRun(
            { ...(input as any), approvalTicket: t ?? input.approvalTicket },
            {}
          ),
        {
          json,
          waitApproval: bool(values['wait-approval']),
          pollTimeoutMs: num(values['poll-timeout']) ?? 120_000
        }
      );
    }
    case 'verify': {
    ...
    case 'env': {
      const action = (str(values['action']) ?? 'create') as
        | 'create'
        | 'destroy';
      const input: Record<string, unknown> = {
        action,
    ...
    case 'mcp': {
      if (sub === 'list') {
        ...
        json
          ? jsonOut(servers)
          : servers.forEach((s) =>
              out(`${c('cyan', s.name)} ${s.status} tools=${s.toolCount}`)
            );
        return 0;
      }
      if (sub === 'add') {
      ...
      if (sub === 'preset') {
        const r = await client.connectMcpPreset(
          str(values['id']) ?? '',
          str(values['mcp-token'])
        );
        json ? jsonOut(r) : out(c('green', `已接入预设 ${r.server.name}`));
        return 0;
      }
      err(c('red', '用法: ah mcp list|add|preset ...'));
      return 1;
    }
    case 'approvals': {
      if (sub === 'list') {
        const { tickets } = await client.listApprovals(
          str(values['status']) as any
        );
        json
          ? jsonOut(tickets)
          : tickets.forEach((t) =>
              out(`${c('yellow', t.id)} ${t.action} ${t.status}`)
            );
        return 0;
      }
      if (sub === 'decide') {
        ...
        if (!id || !decision) {
          err(c('red', '需 --id 与 --decision approve|reject'));
          return 1;
        }
        const r = await client.decideApproval(
          id,
          decision,
          str(values['by']) ?? 'cli'
        );
        json
          ? jsonOut(r)
          : out(c('green', `工单 ${r.ticket.id} → ${r.ticket.status}`));
        return 0;
      }
      err(c('red', '用法: ah approvals list|decide ...'));
      return 1;
    }
    case 'recipes': {
      if (sub === 'list') {
        ...
        json
          ? jsonOut(recipes)
          : recipes.forEach((r) => out(`${c('cyan', r.id)} ${r.name}`));
        return 0;
      }
      if (sub === 'save') {
        const r = await client.saveRecipe({
          jobId: str(values['job']) ?? '',
          name: str(values['recipe-name'])
        });
        json ? jsonOut(r) : out(c('green', `已保存配方 ${r.recipe.id}`));
        return 0;
      }
      err(c('red', '用法: ah recipes list|save ...'));
      return 1;
    }
    case 'eval': {
      const jobId = str(values['job']);
      if (!jobId) {
        err(c('red', '需 --job JOBID'));
        return 1;
      }
    ...
    case 'memory': {
      ...
      if (sub === 'clear') {
        ...
        json ? jsonOut(r) : out(c('green', `已清空 ${r.sessionKey}`));
        return 0;
      }
      err(c('red', '用法: ah memory get|clear --session KEY'));
      return 1;
    }
    default:
...
main()
  ...
  .catch((e) => {
    err(c('red', `致命错误: ${(e as Error).message}`));
    process.exitCode = 1;
  });
