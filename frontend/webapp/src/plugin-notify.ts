...
import { authedFetch, getUsername } from './api';
...
function startSse(): void {
  // /api/events 走 AccountAuthorizer 双因子（ah_auth Cookie + x-ah-username 头）。
  // 裸 fetch 并从 localStorage 回填登录用户名（若已登录）：这样 401 不会触发 authedFetch
  // 内的 handleUnauthorized → 跳转登录页；SSE 鉴权失败时只降级轮询，保持原有降级语义。
  if (es) return;
  const controller = new AbortController();
  es = { close: () => controller.abort() };
  const username = getUsername();
  const hdrs: Record<string, string> = { accept: 'text/event-stream' };
  if (username) hdrs['x-ah-username'] = username;
