...
# 运行时只需 packages/ui/dist（HTTP+SSE 服务）、packages/core/dist（被 ui 依赖）
...
# - webapp（Vite+Lit）在 `pnpm -r build` 阶段一并构建，ui server 会优先托管
#   packages/webapp/dist；缺省回退到 public/index.html。
...
#   RUN pnpm --filter @agent-harness/ui deploy --legacy --prod /out
# 并在 runtime 仅 COPY --from=build /out ./  （/out 已含 ui + core 运行产物 + 生产依赖）。
# 注意：ui 当前标记 private:true；若使用 deploy 方案，需先评估 pnpm 对 private 包的部署限制，
# 或把 ui 拆分为可发布的“server”包。
