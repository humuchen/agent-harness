...
async function main(): Promise<void> {
  ...
  if (configs.length === 0) {
    ...
    console.log(
      '[multi-mcp] 未检测到 MCP 环境变量，使用示例默认配置（context7 + filesystem）'
    );
  }
  ...
  console.log(
    `[multi-mcp] 已注册 ${tools.schemas().length} 个工具到共享 ToolRegistry`
  );
  ...
  const llm: LLM = process.env.OPEN_API_KEY
    ...
    : async (_m, _t) => ({
        content:
          '（mock）多 MCP server 已按上方日志接入；填入 OPEN_API_KEY 后由真实模型驱动工具调用。',
        tool_calls: []
      });
  ...
  const answer = await agent.run(
    '列出当前目录下的文件，并告诉我 typescript 这个库的 Context7 库 ID。'
  );
  console.log('\n=== 最终回复 ===\n' + answer);
