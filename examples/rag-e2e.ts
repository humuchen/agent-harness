 ...
 *   4. 将检索结果作为上下文融入生成（此处若有 OPEN_API_KEY 走真实 LLM，
 *      否则打印检索上下文并说明融合方式）
 *
...
const RAG_BIN =
  process.env.RAG_BIN ||
  path.resolve(process.cwd(), 'services/rag/dist/index.js');
...
async function main(): Promise<void> {
  ...
  console.log(
    `  检索 ${resp.results.length} 条，trace_id=${resp.trace_id}，延迟 ${resp.latency_ms}ms`
  );
  resp.results.forEach((r, i) =>
    console.log(
      `  [${i + 1}] (${r.doc_id}, score=${r.score.toFixed(3)}) ${r.content}`
    )
  );
  ...
  if (process.env.OPEN_API_KEY) {
    ...
    console.log(
      '\n[演示] 将上述片段作为 tool 消息回灌 LLM，即可生成带 [1][2] 引用的回答。\n' +
        '（设置 OPEN_API_KEY 可走真实 LLM 生成；生产环境由 agent-harness 核心 loop 自动完成）'
    );
  }
