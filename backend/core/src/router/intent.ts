 ...
 * - INTENT_ROUTER=auto 智能降级：有 OPEN_API_KEY 用 llm（精准），否则用 rule（离线可用），
...
const ACTION_CAPABILITY_KEYWORDS: Array<{
  keys: string[];
  capability: string;
}> = [
  {
  ...
  {
    keys: ['执行', '命令', '运行脚本', 'shell', '脚本', 'terminal', '部署'],
...
function labelIntent(prompt: string): string {
  ...
  if (/(查询|搜索|搜一下|查一下|检索|search|lookup|find)/.test(p))
    return 'lookup';
  if (
    /(计算|算一下|运行|执行|生成|写|帮我做|帮我整理|create|generate|build|run)/.test(
      p
    )
  )
    return 'task';
...
function classifyByRule(prompt: string): Intent {
  ...
  let bestScore = 0;
  for (const [domain, keys] of Object.entries(DOMAIN_KEYWORDS) as [
    Exclude<IndustryDomain, 'generic'>,
    string[]
  ][]) {
    let score = 0;
    for (const k of keys) {
 ...
 * - `auto`：有 OPEN_API_KEY → 'llm'（精准）；否则 → 'rule'（离线可用）—— 智能降级。
 * - 其它 / 缺省 → 'rule'。
 */
export function resolveIntentMode(
  ...
  const raw = (requested ?? env.INTENT_ROUTER ?? 'rule')
    .toString()
    .trim()
    .toLowerCase();
  if (raw === 'llm') return 'llm';
  if (raw === 'auto') return env.OPEN_API_KEY ? 'llm' : 'rule';
  return 'rule';
}

/**
 * 意图路由器。
 * - `mode` 来自 env INTENT_ROUTER：`rule`（默认）/ `llm`（小模型分类，需 OPEN_API_KEY）/
...
export class IntentRouter {
  ...
  constructor(
    opts: { mode?: 'rule' | 'llm' | 'auto'; cacheSize?: number } = {}
  ) {
  ...
  async classify(prompt: string): Promise<Intent> {
    ...
    let intent: Intent;
    if (this.mode === 'llm') {
      intent = await this.classifyByLlm(prompt).catch(() =>
        classifyByRule(prompt)
      );
    } else {
