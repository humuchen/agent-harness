...
export function buildPlannerPrompt(userInput: string): string {
  return [
    ...
    '4b. 文件交付对齐：若用户目标要求生成 PPT / Excel / CSV 等真实文件，把「生成交付文件」拆为独立任务（title 注明格式，如「生成 Excel 汇总表」），expectedOutput 写明将交付的文件名与内容结构；执行环境若具备 doc_export/deliver_file 工具则产出真实文件，否则在该任务正文中给出完整表格 / 大纲文本兜底。outputChecks 相应使用主题词（如「汇总表」「交付文件」）。',
    '',
