...
test('P4.8 重试预算守卫：剩余预算不足以跑完一轮时跳过自检重试，第一轮产出不被超时提示覆盖', async () => {
  ...
  const harness = makeHarness({
    // 总预算 8s → 重试门槛收敛为 min(60s, 2000ms) = 2000ms。首轮耗时 6.5s 后剩余
    // 约 1.5s < 2s，与真实场景（第一轮吃掉大半预算）同形。
    // 时序余量说明：此前 1s/850ms 的紧凑比例只给事件循环抖动留了 150ms——全量并行
    // 跑测试时调度抖动可超限，首轮被硬超时掐断导致用例假红（已多次偶现）。等比放大
    // 到 8s/6.5s 后，抖动容差提升到 1.5s，语义不变（首轮吃掉 ~81% 预算）。
    timeoutMs: 8000,
    llm: async () => {
      calls += 1;
      if (calls === 1) {
