...
test('resolveIntentMode：显式 llm → llm（忽略 env key）', () => {
  assert.strictEqual(resolveIntentMode('llm', { OPEN_API_KEY: '' }), 'llm');
});

test('resolveIntentMode：显式 rule → rule（即使有 key）', () => {
  assert.strictEqual(
    resolveIntentMode('rule', { OPEN_API_KEY: 'sk-x' }),
    'rule'
  );
});

test('resolveIntentMode：auto + 有 OPEN_API_KEY → llm（精准）', () => {
  assert.strictEqual(
    resolveIntentMode('auto', { OPEN_API_KEY: 'sk-x' }),
    'llm'
  );
});

test('resolveIntentMode：auto + 无 OPEN_API_KEY → rule（离线可用）', () => {
  assert.strictEqual(resolveIntentMode('auto', {}), 'rule');
  assert.strictEqual(resolveIntentMode('auto', { OPEN_API_KEY: '' }), 'rule');
});
...
test('resolveIntentMode：env INTENT_ROUTER=auto 收敛为 rule（无 key）', () => {
  assert.strictEqual(
    resolveIntentMode(undefined, { INTENT_ROUTER: 'auto' }),
    'rule'
  );
});
...
test('resolveIntentMode：env INTENT_ROUTER=llm → llm', () => {
  assert.strictEqual(
    resolveIntentMode(undefined, { INTENT_ROUTER: 'llm' }),
    'llm'
  );
});
...
test('resolveIntentMode：未知值 → rule（稳健降级）', () => {
  ...
  assert.strictEqual(
    resolveIntentMode(undefined, { INTENT_ROUTER: 'weird' }),
    'rule'
  );
});
...
test('IntentRouter：mode=auto + 有 key → activeMode llm', () => {
  const prev = process.env.OPEN_API_KEY;
  process.env.OPEN_API_KEY = 'sk-test';
  try {
    assert.strictEqual(new IntentRouter({ mode: 'auto' }).activeMode, 'llm');
  } finally {
    if (prev === undefined) delete process.env.OPEN_API_KEY;
    else process.env.OPEN_API_KEY = prev;
  }
});
