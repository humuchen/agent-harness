...
export interface ChatMessage {
  ...
  /** 纯前端 UI 态：调用链路「消息上下文」面板内本条消息是否展开（受「折叠全部」统一控制，
   *  持久化在消息对象上以防抽屉重渲染丢失展开态）。 */
  expanded?: boolean;
...
export interface ServerState {
  openrouter: boolean;
  /**
   * 当前登录用户的 LLM 就绪状态（BYOK，per-user）。
   * ready=true 表示已配置可用 Key（用户级 / 平台兜底），可发起真实对话；
   * source 标记来源档位；未登录或旧服务端无该字段时按 ready=false 处理。
   */
  llm?: {
    ready: boolean;
