...
> **落地状态**：本设计已**实现**为 `plugins/customer-service`（`@agent-harness/customer-service`，19 个源文件）。
...
async function search_faq(args: {
  query: string;
  topK?: number;
}): Promise<{ hits: FaqHit[] }>;
async function transfer_to_human(args: {
  reason: string;
  summary: string;
...
| 4    | 生产化                     | M6(共享后端/向量 FAQ), M5(KB-MCP), M1(企微/飞书通知), k8s 多副本 |
