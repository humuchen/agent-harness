...
2. **输入护栏**：`checkInput` 扫描密钥 / 提示词注入 / 超长输入。被拦截则 `emit guardrail:blocked` 并直接 `return`，不进入循环。
3. **记忆加载**：若 `memory.hasPersistence`，从 `FileMemoryStore` / `SqliteMemoryStore` 加载长期笔记。
4. **系统提示注入**：把长期记忆（`memory.systemContext()`）拼进系统提示词，再 `memory.add` 系统 + 用户消息。
5. **主循环**（`for step < maxSteps`，默认 24）：
...
6. **收尾**：`memory.save()`（持久化）→ `redactOutput`（出口 PII 脱敏）→ `emit run:end` → `return final`。
...
| **onEvent / 可观测** | 全程 | `emit`：run:start/step:start/llm:call/llm:response/tool:start/tool:result/run:cost/guardrail:blocked/budget:exceeded/run:end/error；`telemetry`：`incCounter`/`recordTokens`/`recordCost`/`recordError`/`withSpan` |
