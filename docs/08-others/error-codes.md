...
**错误信息**: `真实模式需要有效的 LLM API Key：请到「设置 → 模型服务商」填入你的 OpenRouter Key，或在「模型」面板添加带 Key 的自定义模型。`

**位置**: `access/server/src/runner.ts`（BYOK 改造后，凭据由 server 按用户解析后 per-run 注入，不再读 `process.env.OPEN_API_KEY`）

**背景（BYOK）**: 从本版本起 LLM 调用凭据不再由环境统一配置。每个登录用户需到「设置 → 模型服务商」填入自己的 OpenRouter / OpenAI Key，按用户加密落库，运行期按用户注入。环境不再写死 `OPEN_API_KEY`，因此该错误意味着「当前用户未配置可用 Key」，而非「服务端缺环境变量」。

**解决**:

```bash
# 方式1（推荐）：用户在「设置 → 模型服务商」自填 Key（per-user，跨设备可用）
# 方式2：在「模型」面板添加带 Key 的自定义模型（任意 OpenAI 兼容端点）
# 方式3（仅演示/灰度）：管理员开启平台兜底
#   ALLOW_PLATFORM_KEY=true
#   PLATFORM_OPEN_API_KEY=sk-or-...
```

---

### `provider_key_required` · HTTP 402

**含义**: real / real-mcp 模式下，当前登录用户未配置任何可用的 LLM API Key，且平台兜底（`ALLOW_PLATFORM_KEY`）未开启，`resolveRunCredential` 返回 `source: none`。

**触发点**: `access/server/src/server.ts` 的 `/api/run`（在 startSse 之前以结构化 JSON 拒绝）。

**客户端处理**: 拦截发送，提示「请到「设置 → 模型服务商」填入你的 OpenRouter Key」，并 dispatch `ah-goto: settings` 打开设置面板。

**解决**: 用户在设置页保存有效 Key；或管理员开启平台兜底（仅演示/灰度）。

---

### `provider_key_invalid` · HTTP 401

**含义**: 运行期携带的 Key 被上游（OpenRouter / OpenAI）拒绝（401/403）；或由 `/api/account/provider-keys/:provider/verify` 回写 `status: invalid`。

**触发点**: LLM 适配层 `callOpenAIChat` 返回上游鉴权失败；verify 端点回写状态。

**客户端处理**: 提示「Key 已失效，请更新」，并将设置面板该 provider 状态置红。

**解决**: 用户在设置页重新填入有效 Key 并测试连通（verify）。
