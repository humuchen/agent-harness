...
Use the repository's isolated TypeScript toolchain (the sandbox `pnpm install` is blocked by a safe-delete guard, so compile with the pre-provisioned managed `tsc`):
