...
- 部署：Dockerfile(非root ah) / docker-compose(+redis) / render.yaml(free/oregon, healthCheck /api/state) / deploy/k8s

## 关键约定
- 跨包解析：tsconfig paths 把 @agent-harness/core 指向 backend/core/dist/index.d.ts；`pnpm -r build` 拓扑序先 core 后 server/webapp/plugins。
- 一切降级：无 OPENROUTER_API_KEY→mock；无 HARNESS_API_KEY→dry-run；无 MCP→跳过。
...
## 记忆后端
volatile / file(每会话JSON,原子rename) / sqlite(node:sqlite)，按 MEMORY_BACKEND 选；多副本共享用 RWX 卷 + file 后端（sqlite 网络 FS 锁不可靠）。
